...
export async function initGoals(api: AdminAPI) {
  ...
  // The holiday season is upon us and the winter solstice approaches. Last year, Mina wanted to fight against the encroaching ice and changing world. This year she’s acting more festive. I think she worked out a deal with someone behind the scenes.

  // Rather than donating candles, she recommends you just donate $MUSU. She’ll handle the lighting ceremony.
  ...
  //     250000
  //   );
  //   await api.goal.add.reward.display(10, 'Burning Goat');
  //   await api.goal.add.reward.basic(10, 'Bronze', 1500, 'ITEM', 11233, 10);
  //   await api.goal.add.reward.basic(10, 'Silver', 3000, 'ITEM', 11233, 15);
  //   await api.goal.add.reward.basic(10, 'Gold', 9999, 'ITEM', 11233, 25);
  //   await api.goal.add.reward.basic(10, 'Gold', 9999, 'ITEM', 21005, 1);
  //   await api.goal.enable(10);

  //   await api.goal.create(
  //     11,
  //     'New Celebrations',
  //     `Merry Christmas, happy holidays, and say hello to Baby New Year. I’ve yet to introduce myself, but that can wait for a face to face meeting. Until then, just know I’m someone who wants you to enjoy yourself. Kick back. Put a movie on the second monitor. The world is yours.

  // That television in the Techno Temple has been hissing with static for too long. If you’re already going to mess around in there, you might as well get it working. Send some $MUSU and I’ll take care of it.
  ...
  //     250000
  //   );
  //   await api.goal.add.reward.display(11, 'Movie Night');
  //   await api.goal.add.reward.basic(11, 'Bronze', 1500, 'ITEM', 11413, 10);
  //   await api.goal.add.reward.basic(11, 'Silver', 3000, 'ITEM', 11413, 15);
