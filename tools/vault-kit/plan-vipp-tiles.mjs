...
const existing = new Set(); // fresh VIPP market
const plan = [];
const skipped = [];
for (const n of nodes) {
  ...
  if (!drops.includes("VIPP")) { skipped.push({ ...entry, why: "non-VIPP drop (" + drops.join(",") + ")" }); continue; }
...
writeFileSync(join(here, "vipp-tile-plan.json"), JSON.stringify(out, null, 2));
